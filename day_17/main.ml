...
let neighbors (x, y, z, w) =
  ...
  if dx = 0 && dy = 0 && dz = 0 && dw = 0 then [] else [ x + dx, y + dy, z + dz, w + dw ]
;;
...
let simulate initial_grid ~n =
  let active_points = Hash_set.create (module Point4) in
  for x = 0 to Array.length initial_grid - 1 do
    for y = 0 to Array.length initial_grid.(0) - 1 do
      if initial_grid.(x).(y) then Hash_set.add active_points (x, y, 0, 0)
    done
  done;
  let active_points = ref active_points in
  for _ = 1 to n do
    let new_active_points = Hash_set.create (module Point4) in
    let neighbors_of_active_points =
      !active_points
      |> Hash_set.to_list
      |> List.concat_map ~f:neighbors
      |> Set.of_list (module Point4)
    in
    let points_to_check =
      (!active_points |> Hash_set.to_list) @ (neighbors_of_active_points |> Set.to_list)
      |> Set.stable_dedup_list (module Point4)
    in
    List.iter points_to_check ~f:(fun point ->
      let neighbor_count =
        List.count (neighbors point) ~f:(Hash_set.mem !active_points)
