...
let run_internal t ~get_input ~output =
...
let run' t ~input =
  let output_r, output_w = Pipe.create () in
  let done_ =
    run_internal t ~get_input:(fun () -> Mvar.take input) ~output:(Pipe.write output_w)
  in
  don't_wait_for
    (let%map () = done_ in
     Pipe.close output_w);
