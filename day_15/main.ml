...
let play numbers =
  (* Build list of (number, most_recent_index) *)
  Sequence.unfold_with_and_finish
    ...
    ~running_step:(fun (i, most_recent_index, _last_number_most_recent_index) number ->
      Yield
        ( (number, Map.find most_recent_index number)
        , ( i + 1
...
let a () =
  ...
  let ans = play input |> Sequence.map ~f:fst |> Fn.flip Sequence.nth_exn 2019 in
...
let b () =
  let%bind input = Lazy_deferred.force_exn Input.t in
  let ans = play input |> Sequence.map ~f:fst |> Fn.flip Sequence.nth_exn 29999999 in
  print_s [%sexp (ans : int)];
  return ()
;;
