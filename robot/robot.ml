...
type _ t =
  | With_dir :
      ...
      -> [> `dir ] t
  | Without_dir : { mutable loc : Point.t } -> [ `dummy ] t
[@@deriving sexp_of]
...
let step_forward (With_dir t : [> `dir ] t) = t.loc <- Point.add t.loc t.dir
let step_backward (With_dir t : [> `dir ] t) = t.loc <- Point.add t.loc (Dir.opp t.dir)
...
let dir (With_dir t : [> `dir ] t) = t.dir
let turn (With_dir t : [> `dir ] t) which_way = t.dir <- Dir.turn t.dir which_way
