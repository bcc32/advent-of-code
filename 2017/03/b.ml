...
let iter_coordinates ~f =
  let rec loop_ring x y =
    let ring = x in
    for y = y to y + (2 * ring) - 1 do
      f x y
    done;
    let y = y + (2 * ring) - 1 in
    for x = x - 1 downto x - (2 * ring) do
      f x y
    done;
    let x = x - (2 * ring) in
    for y = y - 1 downto y - (2 * ring) do
      f x y
    done;
    let y = y - (2 * ring) in
    for x = x + 1 to x + (2 * ring) do
      f x y
    done;
    let x = x + (2 * ring) in
    loop_ring (x + 1) y
...
let () =
  let threshold =
    In_channel.with_file
      (Sys.get_argv ()).(1)
      ~f:(fun file -> In_channel.input_all file |> String.strip |> Int.of_string)
  in
  iter_coordinates ~f:(fun x y ->
    let sum = ref 0 in
    iter_neighbors (x, y) ~f:(fun x2 y2 ->
      Hashtbl.find memory (x2, y2) |> Option.iter ~f:(fun v -> sum := !sum + v));
    if !sum > threshold
    then (
      printf "%d\n" !sum;
      exit 0);
