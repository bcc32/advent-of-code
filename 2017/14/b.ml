...
let hash string =
  let lengths =
    (string |> String.to_list |> List.map ~f:Char.to_int) @ [ 17; 31; 73; 47; 23 ]
  in
  ...
  |> List.map ~f:(fun byte ->
    List.range 0 8 |> List.filter ~f:(fun pos -> byte land (1 lsl (7 - pos)) <> 0))
;;

let () =
  let key =
    In_channel.with_file (Sys.get_argv ()).(1) ~f:In_channel.input_all |> String.strip
  in
  ...
  for i = 0 to 127 do
    for j = 0 to 127 do
      if array.(i).(j) && not (Hash_set.mem visited (i, j))
      then (
        dfs array i j;
        incr count)
    done
