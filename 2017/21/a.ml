...
let is_match pattern grid i j =
  ...
  let subgrid = Array.sub grid ~pos:i ~len |> Array.map ~f:(Array.sub ~pos:j ~len) in
    ...
    for j = 0 to Array.length grid.(0) - 1 do
      ...
      then
        with_return (fun { return } ->
          List.iter rules ~f:(function Two (left, right) | Three (left, right) ->
...
let () =
  let rules =
    ...
    |> List.map ~f:(fun line ->
      ...
      | [ left; right ] ->
        ...
        if Array.length left = 2 then Two (left, right) else Three (left, right)
      | _ -> assert false)
  in
  (* Debug.eprint_s [%message (rules : rule list)]; *)
  ...
  Array.sum
    (module Int)
    !grid
    ~f:(fun row -> Array.count row ~f:(fun x -> Char.( = ) x '#'))
  |> printf "%d\n"
;;
